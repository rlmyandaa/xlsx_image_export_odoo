...
def close(self):
    ...
    if self.use_max_width:
        for col in self.max_width_column:
            self.worksheet.set_column(col, col, width=pixels_to_width(self.max_width))
...
def write_image(self, row, column, base64_source: ImageProcess):
    ...
    self.worksheet.insert_image(row, column, filename, {"image_data": image_data, 'object_position': 1})
...
def _check_xlsx_image_export(self):
    ...
    if enable_xlsx_image_export:
        ...
        self.cell_max_height_px = float(ir_config_param.get_param('xlsx_image_export.cell_max_height_px')) or MAX_HEIGHT_IMAGE_CELL
        self.cell_image_spacing_percent = (float(ir_config_param.get_param('xlsx_image_export.cell_image_spacing_percent')) * 100) or IMAGE_CELL_TOLERATION_PERCENT
